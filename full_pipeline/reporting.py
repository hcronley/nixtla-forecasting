...
def create_top5_timeseries_plot(backtest_results: Dict[str, Any], original_data: pd.DataFrame, top_n: int = 5) -> go.Figure:
    """
    Create time series plot showing top N models' predictions on original data.
    ...
    original_data : pd.DataFrame
        Original time series with 'ds' and 'y' columns
    top_n : int
        Number of top models to display (default: 5)
    ...
    ranking_df = comparator.get_ranking_table(top_n=top_n)
    ...
    fig.add_trace(go.Scatter(
        ...
        opacity=0.8,
        hovertemplate='<b>Actual</b><br>%{x|%Y-%m-%d}<br>%{y:.2f}<extra></extra>'
    ))

    # Color palette for top models
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd']

    # Add top 5 models' forecasts from backtest results
    top_models = ranking_df['Model'].head(top_n).tolist()

    for idx, model_name in enumerate(top_models):
        # Get forecasts for this model from all windows
        if 'window_results' in backtest_results:
            all_forecasts = []
            all_dates = []

            for window_idx, window_data in enumerate(backtest_results['window_results']):
                if model_name in window_data:
                    model_window = window_data[model_name]
                    if 'forecasts' in model_window:
                        forecasts_df = model_window['forecasts']
                        all_forecasts.extend(forecasts_df['y_pred'].values)
                        all_dates.extend(forecasts_df['ds'].values)
            ...
            if all_forecasts:
                fig.add_trace(go.Scatter(
                    ...
                    hovertemplate=f'<b>{model_name}</b><br>%{{x|%Y-%m-%d}}<br>%{{y:.2f}}<extra></extra>'
                ))

    # Update layout
    fig.update_layout(
        title='Top 5 Models: Predictions vs Actual Data',
        xaxis_title='Date',
        yaxis_title='Value',
        hovermode='x unified',
        height=500,
